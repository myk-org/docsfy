...
async def generate(request: GenerateRequest):
    ...
    await save_project(name=project_name, repo_url=request.repo_url, status="generating")
...
async def _run_generation(
    repo_url: str, project_name: str, ai_provider: str, ai_model: str, ai_cli_timeout: int,
) -> None:
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            repo_path, commit_sha = await asyncio.to_thread(clone_repo, repo_url, Path(tmp_dir))

            existing = await get_project(project_name)
            if existing and existing.get("last_commit_sha") == commit_sha and existing.get("status") == "ready":
                logger.info(f"Project {project_name} is up to date at {commit_sha[:8]}")
                return

            plan = await run_planner(
                repo_path=repo_path, project_name=project_name,
                ai_provider=ai_provider, ai_model=ai_model, ai_cli_timeout=ai_cli_timeout,
            )

            cache_dir = get_project_cache_dir(project_name)
            pages = await generate_all_pages(
                repo_path=repo_path, plan=plan, cache_dir=cache_dir,
                ai_provider=ai_provider, ai_model=ai_model, ai_cli_timeout=ai_cli_timeout,
            )
        ...
        await update_project_status(
            project_name, status="ready", last_commit_sha=commit_sha,
            page_count=page_count, plan_json=json.dumps(plan),
        )
        ...
        await update_project_status(project_name, status="error", error_message=str(exc))
...
async def download_project(name: str):
    ...
    if project["status"] != "ready":
        raise HTTPException(status_code=400, detail=f"Project '{name}' is not ready (status: {project['status']})")
    site_dir = get_project_site_dir(name)
    if not site_dir.exists():
        raise HTTPException(status_code=404, detail=f"Site directory not found for '{name}'")
    buffer = BytesIO()
    with tarfile.open(fileobj=buffer, mode="w:gz") as tar:
        tar.add(str(site_dir), arcname=name)
    buffer.seek(0)
    return StreamingResponse(
        buffer, media_type="application/gzip",
        headers={"Content-Disposition": f"attachment; filename={name}-docs.tar.gz"},
    )


@app.get("/docs/{project}/{path:path}")
async def serve_docs(project: str, path: str = "index.html"):
    if not path or path == "/":
        path = "index.html"
