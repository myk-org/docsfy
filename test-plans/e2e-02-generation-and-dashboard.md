...
**Verify branches are filtered by the repo URL:**

Enter a different repo URL and verify branches change or disappear:
```shell
agent-browser clear "#gen-repo-url"
agent-browser type "#gen-repo-url" "https://github.com/myk-org/nonexistent-repo"
agent-browser click "#gen-branch"
agent-browser wait 500
agent-browser javascript "document.querySelectorAll('#branch-dropdown .branch-option:not([style*=\"display: none\"])').length"
```

**Expected result:** Returns `0` --- no known branches for this repo.

Restore the original repo URL:
```shell
agent-browser clear "#gen-repo-url"
agent-browser type "#gen-repo-url" "https://github.com/myk-org/for-testing-only"
```
