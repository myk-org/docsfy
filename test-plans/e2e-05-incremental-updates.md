...
```shell
curl -s http://localhost:8800/api/status -H "Authorization: Bearer <TEST_USER_PASSWORD>" | python3 -c "import sys,json; data=json.load(sys.stdin); matches=[p for p in data['projects'] if p['name']=='for-testing-only' and p['branch']=='main' and p['ai_model']=='gemini-2.5-flash']; print(matches[0]['last_commit_sha'][:8] if matches else 'not found')"
```
...
MERGED_SHA=$(gh api "repos/myk-org/for-testing-only/pulls/$PR_NUM" --jq '.merge_commit_sha[:8]')
...
Poll the API every 5 seconds, capturing `current_stage` and `status` values until the generation completes:

```shell
# Poll API every 5s, capture current_stage values
