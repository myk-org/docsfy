...
```shell
agent-browser javascript "fetch('/api/generate', {method:'POST', headers:{'Content-Type':'application/json'}, credentials:'same-origin', body: JSON.stringify({repo_url:'https://github.com/myk-org/for-testing-only', branch:'main', repo_type:'tests'})}).then(r=>r.json().then(body=>({status:r.status,body})))"
```
...
```shell
agent-browser javascript "fetch('/api/generate', {method:'POST', headers:{'Content-Type':'application/json'}, credentials:'same-origin', body: JSON.stringify({repo_url:'https://github.com/myk-org/for-testing-only', branch:'main', repo_type:'invalid'})}).then(r=>r.json().then(body=>({status:r.status,body})))"
```
...
```shell
agent-browser javascript "fetch('/api/generate', {method:'POST', headers:{'Content-Type':'application/json'}, credentials:'same-origin', body: JSON.stringify({repo_url:'https://github.com/myk-org/for-testing-only', branch:'main'})}).then(r=>r.json().then(body=>({status:r.status,body})))"
```
...
Click the first ready variant in the sidebar tree, then:
...
```shell
docsfy generate https://github.com/myk-org/for-testing-only --repo-type invalid --branch main; echo "exit_code=$?"
```
